  ...
  {   // Receive SYN ACK
    ...
    if (recvlen > 0)
    {
      ...
      if (rec.getAck() && rec.getSyn() &&
          rec.getAckNumber() == (initWindow[0].getSeqNumber()+1))
      {
...
void receiveFile(int sockfd, struct sockaddr_in addr)
{
  ...
  while (1)
  {
    ...
    if (recvlen > 0)
    {
      ...
      ack.setSeqNumber((rec.getAckNumber()) % MAXSEQ);
      ack.setAckNumber(rec.getSeqNumber() + rec.getLen() % MAXSEQ);
      ack.setFlags(1, 0, 0);
