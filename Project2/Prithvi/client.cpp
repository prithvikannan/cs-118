...
void initiateConnection(int sockfd, struct sockaddr_in addr, string fileName) {
  ...
  cout << "SEND " << p.getSeqNumber() << " " << p.getAckNumber() <<  " SYN" << endl;
  ...
  while (1) {
    ...
    if (recvlen > 0) {
      ...
      cout << "RECV " << rec.getSeqNumber() << " " << rec.getAckNumber()+1 << " " << rec.flagString() << endl;
      if (rec.getAck() && rec.getSyn() &&
        ...
        if (!initWindow[1].isSent()) {
          ...
          cout << "SEND " << sendFilename.getSeqNumber() << " " << sendFilename.getAckNumber() << " " << sendFilename.flagString() << endl;
          ...
          cout << "SEND " << initWindow[1].getSeqNumber() << " " << initWindow[1].getAckNumber() 
              << " " << initWindow[1].flagString() << endl;
          if (sendto(sockfd, &packet, MSS, 0, (struct sockaddr *)&addr,
                     sizeof(addr)) < 0) {
            throwError("Could not send to the server");
          }
    ...
    for (int i = 0; i < 2; i++) {
      if (initWindow[i].isSent() && initWindow[i].isAcked()) {
        ...
        if (i == 0) {
          ...
          cout << initWindow[1].getSeqNumber() << " " << initWindow[1].getAckNumber() 
              << " " << initWindow[1].flagString();
        }
...
void receiveFile(int sockfd, struct sockaddr_in addr) {
  ...
  while (1) {
    ...
    if (recvlen > 0) {
      ...
      rec.convertBufferToPacket(buf);
      cout << "RECV " << rec.getSeqNumber() << " " << rec.getAckNumber() << " " << rec.flagString() << endl;
      if (rec.getFin()) {
      ...
      if (dup == 0) {
        cout << "SEND " << ack.getSeqNumber() << " " << ack.getAckNumber() << " " << ack.flagString() << endl;
      } else if (dup == 1) {
        cout << "SEND " << ack.getAckNumber() << " Retransmission"
             << endl;
      }
      if (sendto(sockfd, &packet, MSS, 0, (struct sockaddr *)&addr,
                 sizeof(addr)) < 0) {
      ...
      if (dup == 0) {
        ...
        if (numPacketsToWriteToFile > 0) {
          ...
          rec.getData(data);
          movingPackWind[RECEIVEWINDOW - 1].setData(data, rec.getLen());
          movingPackWind[RECEIVEWINDOW - 1].setAcked();
          lastWindSeq = movingPackWind[RECEIVEWINDOW - 1].getSeqNumber();
        }
      }
    }
...
void closeConnection(int sockfd, struct sockaddr_in addr) {
  ...
  cout << "SEND "  << ackPacket.getSeqNumber() << " " << ackPacket.getAckNumber() << " " << ackPacket.flagString() << endl;
  if (sendto(sockfd, &packet, MSS, 0, (struct sockaddr *)&addr, sizeof(addr)) <
      0) {
    throwError("Could not send to the server");
  }
  ...
  cout << "SEND "  << finPacket.getSeqNumber() << " " << finPacket.getAckNumber() << " " << finPacket.flagString() << endl;
  if (sendto(sockfd, &packet, MSS, 0, (struct sockaddr *)&addr, sizeof(addr)) <
      0) {
    throwError("Could not send to the server");
  }
  ...
  while (1) {
    ...
    if (recvlen > 0) {
      ...
      cout << "RECV " << recPacket.getSeqNumber() << " " << recPacket.getAckNumber() << " " << recPacket.flagString() << endl;
      if (recPacket.getFin() && recPacket.getSeqNumber() == finSeqNum) {
        // Duplicate fin from the server. Resend the ack for it
        ackPacket.convertPacketToBuffer(packet);
        cout << "SEND " << ackPacket.getAckNumber()
             << " Retransmission " << endl;
        if (sendto(sockfd, &packet, MSS, 0, (struct sockaddr *)&addr,
                   sizeof(addr)) < 0) {
          throwError("Could not send to the server");
    ...
    if (finPacket.hasTimedOut(1) && !finPacket.hasTimedOut(2) &&
        !hasBeenReSent) {
      cout << "SEND " << finPacket.getAckNumber()
           << " Retransmission "
           << " FIN " << endl;
      if (sendto(sockfd, &packet, MSS, 0, (struct sockaddr *)&addr,
                 sizeof(addr)) < 0) {
...
int main(int argc, char *argv[]) {
  ...
  // cout << "Done making connection" << endl;
    ...
    // cout << "Starting recieve file" << endl;
